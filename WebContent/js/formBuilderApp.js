angular.module('formBuilderApp', [ 'threeCanvas' ]).directive(
    'building',
    function() {
      return {
        require : '^threeCanvas',
        ...
        link : function(scope, element, attrs, threeCanvasCtrl) {
          ...
          threeCanvasCtrl.scene.add(makeWallMeshRotateMove(0,
              new THREE.Vector3(-size[0] / 2, 0, size[2] / 2), size[0],
              size[1], scope.config.wallThickness, wallMaterial));
          threeCanvasCtrl.scene.add(makeWallMeshRotateMove(Math.PI / 2,
              new THREE.Vector3(size[0] / 2, 0, size[2] / 2), size[2], size[1],
              scope.config.wallThickness, wallMaterial));
          threeCanvasCtrl.scene.add(makeWallMeshRotateMove(Math.PI,
              new THREE.Vector3(size[0] / 2, 0, -size[2] / 2), size[0],
              size[1], scope.config.wallThickness, wallMaterial));
          threeCanvasCtrl.scene.add(makeWallMeshRotateMove(3 * Math.PI / 2,
              new THREE.Vector3(-size[0] / 2, 0, -size[2] / 2), size[2],
              size[1], scope.config.wallThickness, wallMaterial));
        },
        replace : true
      };
